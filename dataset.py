...
class DatasetGenerator():
    ...
    # stringi sayısal sınıflara dönüştür
    def text_to_labels(self, text):
        return self.label_set.index(text)
    
    # sayısal sınıfları stringe dönüştür
    ...
    def read_wav_file(self, x):
        # scipy wavfile.read kullanılarak wav dosyaları okunur.
        _, wav = wavfile.read(x) 
        # Normalize
        #float32 tipindeki veriler int16nın max değerine bölünür. (int16 aralığı: -32,768, +32,767)
    ...
    def process_wav_file(self, x, threshold_freq=5500, eps=1e-10):
        ...
        # Sample rate
        L = self.sample_rate
        # samplerate den uzunsa rastgele kes
        if len(wav) > L:
            i = np.random.randint(0, len(wav) - L)
            wav = wav[i:(i+L)]  
        # samplerate den kısaysa then randomly add silence
        elif len(wav) < L:
            rem_len = L - len(wav)
        ...
        # Create spectrogram using discrete FFT (change basis to frequencies) / ayrık fourier dönüşümü
        # nperseg:segment boyutu, noverlap:iki segment arasında çakışacak nokta sayısı, nfft:fft uzunluğu
